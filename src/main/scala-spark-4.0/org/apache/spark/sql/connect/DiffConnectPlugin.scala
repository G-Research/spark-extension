...
class DiffConnectPlugin extends RelationPlugin {
  ...
  override def transform(bytes: Array[Byte], planner: SparkConnectPlanner): Optional[LogicalPlan] = {
    val relation = protobuf.Any.parseFrom(bytes)
    ...
    Optional.of(left.diff(right, options, JavaConverters.iterableAsScalaIterableConverter(diff.getIdColumnList()).asScala.toSeq, JavaConverters.iterableAsScalaIterableConverter(diff.getIgnoreColumnList()).asScala.toSeq).logicalPlan)
  }
}
