...
```shell script
pip install pyspark-extension==2.14.2.3.4
```
...
spark = SparkSession \
    .builder \
    .config("spark.jars.packages", "uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.4") \
    .getOrCreate()
```
...
```shell script
pyspark --packages uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.4
```
...
```shell script
spark-submit --packages uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.4 [script.py]
```
...
add **a jar dependency** to your notebook using these **Maven coordinates**:
    ...
    uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.4
