...
```sbt
libraryDependencies += "uk.co.gresearch.spark" %% "spark-extension" % "2.14.2-3.5"
```
...
```groovy
dependencies {
    implementation "uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5"
}
```
...
```shell script
spark-submit --packages uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5 [jar]
```
...
```shell script
spark-shell --packages uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5
```
...
spark = SparkSession \
    .builder \
    .config("spark.jars.packages", "uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5") \
    .getOrCreate()
```
...
```shell script
pyspark --packages uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5
```
...
```shell script
spark-submit --packages uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5 [script.py]
```
...
```shell script
pip install pyspark-extension==2.14.2.3.5
```
...
add **a jar dependency** to your notebook using these **Maven coordinates**:
    ...
    uk.co.gresearch.spark:spark-extension_2.12:2.14.2-3.5
