...
|`DiffComparators.string(whitespaceAgnostic=true)`|With `whitespaceAgnostic=true` (the default), two `StringType` values are compared while ignoring white space differences. For this comparison, sequences of whitespaces are collapesed into single whitespaces, leading and trailing whitespaces are removed. With `whitespaceAgnostic=false`, string values are compared by the default comparator|
